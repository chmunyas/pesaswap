-- PESASWAP gift cards migration — adds lifecycle/audit/MNO fields.
--
-- Combines the schema patterns from:
...
--
-- Idempotent caveats:
--   - The ALTER TABLE will fail if columns already exist. For dev environments
--     re-running the migration after a partial apply will need a manual cleanup
--     or a versioned migration tool. PESASWAP is dev-only; a real prod install
--     should wrap this in a real CodeIgniter4 migration class. Run during a
--     maintenance window — adding 12 columns + 3 indexes will briefly lock
--     ospos_giftcards.
--   - CREATE TABLE IF NOT EXISTS + the NOT EXISTS backfill ARE re-runnable.
